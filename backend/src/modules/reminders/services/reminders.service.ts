...
export class RemindersService {
  ...
  private async queueReminder(reminder: Reminder, delay: number = 0): Promise<void> {
    const job = await this.reminderQueue.add(
      ...
      },
      {
        ...
        backoff: {
          type: 'exponential',
          delay: this.configService.get<number>('QUEUE_JOB_BACKOFF_DELAY', 60000),
        },
        removeOnComplete: {
  ...
  async checkDueInvoices(): Promise<void> {
    ...
    try {
      const dueSoonDays = this.configService.get<number>('REMINDER_DUE_SOON_DAYS', 3);
  ...
  async checkOverdueInvoices(): Promise<void> {
    ...
    try {
      const overdueIntervalDays = this.configService.get<number>('REMINDER_OVERDUE_INTERVAL_DAYS', 7);
