...
export class AuthService {
  ...
  async registerCompany(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const existingUser = await this.usersService.findByEmail(registerDto.owner.email);
      if (existingUser) {
      ...
      // Generate tokens
      const tokens = await this.generateTokens(user.id, company.id, user.email, user.role, false, metadata);
  ...
  async refreshTokens(
    ...
    const { accessSession, refreshSession } = await this.sessionService.refreshSession(
      payload.sessionId,
      metadata,
    );
  ...
  async validateAccessToken(token: string): Promise<UserSession> {
    ...
    const session = await this.sessionService.validateAccessSession(payload.sessionId);
