    ...
    for (const invoice of paidInvoices) {
      ...
      if (isFullPayment) {
        ...
        const paymentData = {
          ...
          metadata: {
            channel: faker.helpers.arrayElement(['M-Pesa', 'Bank Transfer', 'Cash', 'Card']),
        ...
        for (let i = 0; i < paymentCount && remainingAmount > 0; i++) {
          ...
          const toDate = invoice.paidDate ? new Date(invoice.paidDate) : new Date();
          ...
          const paymentData = {
            ...
            reference: `PAY-${faker.string.alphanumeric(10).toUpperCase()}`,
            metadata: {
              channel: faker.helpers.arrayElement(['M-Pesa', 'Bank Transfer', 'Cash']),
              processedBy: faker.person.fullName(),
            },
